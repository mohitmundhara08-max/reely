...
const C = {
  bg:"#0a0a0a", s1:"#111111", s2:"#181818", border:"#242424",
  pink:"#FF2D78", yellow:"#FFE500", mint:"#00F0C8", purple:"#9B5DE5",
  blue:"#4CC9F0", orange:"#FF6B35", white:"#F5F5F5", muted:"#666", dim:"#333",
};
  ...
  <svg width={size} height={size} viewBox="0 0 24 24" style={style}>
    {[0,45,90,135].map(a=>(
      <line key={a} x1="12" y1="2" x2="12" y2="22"
        stroke={color} strokeWidth="2.5" strokeLinecap="round" transform={`rotate(${a} 12 12)`}/>
    ))}
  </svg>
);
...
async function callAI(messages, system) {
  const r = await fetch(`${BOT_URL}/ai`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ messages, system }),
  });
  const d = await r.json();
  return d.content?.[0]?.text ?? "Error — check that ANTHROPIC_API_KEY is set in Railway.";
}
    ...
    <div style={{background:C.s1,borderTop:`1px solid ${C.border}`,padding:"14px 16px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {QUICK.map(a=>(
            <button key={a.id} onClick={()=>a.id==="free"?setFree(true):send(a.prompt)}
              style={{fontSize:11,padding:"5px 12px",background:"transparent",border:`1px solid ${C.mint}44`,borderRadius:20,color:C.mint,cursor:"pointer",fontFamily:"monospace"}}>
              {a.label}
            </button>
          ))}
        </div>
      ...
      {msgs.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:240,overflowY:"auto",scrollbarWidth:"none"}}>
          ...
          {busy&&<span style={{fontSize:12,fontFamily:"monospace",color:C.mint}}>thinking...</span>}
          <div ref={endRef}/>
        </div>
      )}
      {(msgs.length>0||free)&&(
        <div style={{display:"flex",gap:8}}>
          <input autoFocus value={inp} onChange={e=>setInp(e.target.value)}
            ...
            style={{flex:1,fontSize:13,background:C.s2,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 14px",color:C.white,outline:"none",fontFamily:"monospace"}}/>
          <button onClick={()=>send(inp)} disabled={busy||!inp.trim()}
            style={{background:C.mint,border:"none",borderRadius:10,padding:"0 16px",color:"#000",fontWeight:700,cursor:"pointer",fontSize:14}}>→</button>
        </div>
      )}
    </div>
  );
...
function Btn({label,color,onClick,style={}}) {
  return (
    <button onClick={onClick}
      style={{fontSize:11,fontFamily:"monospace",padding:"5px 12px",borderRadius:20,background:"transparent",border:`1px solid ${color}55`,color,cursor:"pointer",transition:"all 0.15s",...style}}
...
function ReelCard({reel,onStatus,onDelete,idx=0}) {
  ...
  const accent=pickColor(reel.url); const dl=fmtDate(reel.reminder);
  return (
    <div style={{background:C.s1,border:`1px solid ${C.border}`,borderLeft:`3px solid ${accent}`,borderRadius:14,overflow:"hidden",opacity:reel.status==="done"?0.45:1,animation:`cardIn 0.3s ${idx*0.05}s ease both`,transition:"transform 0.15s,box-shadow 0.15s"}}
      ...
      <div onClick={()=>{setOpen(!open);if(!open)setAi(false);}} style={{padding:"12px 14px",cursor:"pointer",display:"flex",flexDirection:"column",gap:7}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:10,fontFamily:"monospace",fontWeight:700,color:accent,background:accent+"18",border:`1px solid ${accent}44`,padding:"2px 8px",borderRadius:20}}>{plat(reel.url)}</span>
        ...
        {reel.note&&<div style={{fontSize:13,color:"#ccc",lineHeight:1.5,borderLeft:`2px solid ${accent}55`,paddingLeft:10}}>{reel.note}</div>}
        <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`}}>
          <div style={{padding:"10px 14px",display:"flex",flexDirection:"column",gap:10}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>setAi(!ai)} style={{fontSize:11,fontFamily:"monospace",padding:"5px 12px",borderRadius:20,cursor:"pointer",background:ai?C.purple+"33":"transparent",border:`1px solid ${C.purple}`,color:C.purple}}>
                ✦ AI deep dive
              </button>
...
function AddPanel({onAdd,onClose}) {
  ...
  const inp = (v,s,p,mono,onK) => (
    <input value={v} onChange={e=>s(e.target.value)} onKeyDown={onK} placeholder={p}
      style={{background:C.s2,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px",color:C.white,fontSize:13,outline:"none",fontFamily:mono?"monospace":"inherit",width:"100%"}}
      onFocus={e=>e.target.style.borderColor=C.pink} onBlur={e=>e.target.style.borderColor=C.border}/>
  );
  return (
    <div style={{background:C.s1,border:`1px solid ${C.pink}55`,borderRadius:16,padding:"18px 20px",marginBottom:20,display:"flex",flexDirection:"column",gap:12}}>
      ...
      {inp(url,setUrl,"paste any link...",true,e=>e.key==="Enter"&&save())}
      {inp(creator,setCreator,"@creator handle (optional)",false,null)}
      ...
      <div style={{display:"flex",gap:8}}>
        {[{v:"save",l:"just save ✓"},{v:"remind",l:"remind me ◷"}].map(m=>(
          <button key={m.v} onClick={()=>setMode(m.v)} style={{flex:1,padding:"9px",borderRadius:10,cursor:"pointer",fontFamily:"monospace",fontSize:12,fontWeight:700,background:mode===m.v?C.pink:"transparent",color:mode===m.v?"#000":C.muted,border:`1px solid ${mode===m.v?C.pink:C.border}`,transition:"all 0.2s"}}>{m.l}</button>
        ))}
      </div>
      {mode==="remind"&&(
      ...
      <button onClick={save} disabled={!valid} style={{padding:"11px",borderRadius:10,border:"none",cursor:valid?"pointer":"default",background:valid?`linear-gradient(135deg,${C.pink},${C.purple})`:"#222",color:valid?"#fff":C.muted,fontWeight:700,fontSize:14,fontFamily:"monospace"}}>
        save to reely →
      </button>
    </div>
  );
...
function AllView({reels,onStatus,onDelete}) {
  ...
  const filtered=reels.filter(r=>f==="all"||r.status===f).sort((a,b)=>{
  ...
  return <>
    <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
      {["all","saved","done","skipped"].map(s=>{
        ...
        const active=f===s;
        return <button key={s} onClick={()=>setF(s)} style={{fontSize:11,fontFamily:"monospace",fontWeight:700,padding:"5px 14px",borderRadius:20,cursor:"pointer",background:active?colors[s]:"transparent",color:active?"#000":C.muted,border:`1px solid ${active?colors[s]:C.border}`,transition:"all 0.15s"}}>
          {s} <span style={{opacity:0.6}}>{s==="all"?reels.length:reels.filter(r=>r.status===s).length}</span>
        </button>;
      })}
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {filtered.length===0?<EmptyMsg text="nothing here yet~"/>:filtered.map((r,i)=><ReelCard key={r.id} reel={r} onStatus={onStatus} onDelete={onDelete} idx={i}/>)}
    </div>
  </>;
}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        return (
          <div key={creator} style={{background:C.s1,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
            <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:40,height:40,borderRadius:"50%",background:acc+"22",border:`2px solid ${acc}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800,color:acc,flexShrink:0}}>{creator[0].toUpperCase()}</div>
              <div>
              ...
              <div style={{marginLeft:"auto",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                ...
                <div style={{width:60,height:4,background:C.border,borderRadius:99}}>
                  <div style={{width:`${pct}%`,height:"100%",background:acc,borderRadius:99}}/>
                </div>
              </div>
            </div>
      ...
      {none.length>0&&(
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{none.map((r,i)=><ReelCard key={r.id} reel={r} onStatus={onStatus} onDelete={onDelete} idx={i}/>)}</div>
        </div>
      )}
    </div>
  );
...
function RemindersView({reels,onStatus,onDelete}) {
  ...
  return <>
    {overdue.length>0&&<><SectionLabel label="🔥 overdue" color={C.pink}/><div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>{overdue.map((r,i)=><ReelCard key={r.id} reel={r} onStatus={onStatus} onDelete={onDelete} idx={i}/>)}</div></>}
    {upcoming.length>0&&<><SectionLabel label="📅 coming up" color={C.yellow}/><div style={{display:"flex",flexDirection:"column",gap:8}}>{upcoming.map((r,i)=><ReelCard key={r.id} reel={r} onStatus={onStatus} onDelete={onDelete} idx={i}/>)}</div></>}
  </>;
}
...
export default function ReelyApp() {
  ...
  useEffect(()=>{
    try { const s=localStorage.getItem("reely_local"); if(s) setReels(JSON.parse(s)); } catch(_){}
    setLoaded(true);
  },[]);
  useEffect(()=>{
    if(!loaded) return;
    try { localStorage.setItem("reely_local",JSON.stringify(reels)); } catch(_){}
  },[reels,loaded]);

  // Sync from bot every 30s
  ...
  const stats=[
    ...
    ...(reels.filter(r=>r.reminder&&r.status==="saved"&&new Date(r.reminder)<today).length>0?[{v:reels.filter(r=>r.reminder&&r.status==="saved"&&new Date(r.reminder)<today).length,l:"overdue",c:C.pink}]:[]),
  ];
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"'DM Sans',-apple-system,sans-serif"}}>
      ...
      {/* Header */}
      <div style={{position:"sticky",top:0,zIndex:50,background:`${C.bg}ee`,backdropFilter:"blur(12px)",borderBottom:`1px solid ${C.border}`,padding:"0 20px",height:58,display:"flex",alignItems:"center",gap:16}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {stats.map((s,i)=>(
            <span key={i} style={{fontSize:11,fontFamily:"monospace",fontWeight:700,color:s.c,background:s.c+"15",padding:"3px 10px",borderRadius:20,border:`1px solid ${s.c}33`}}>{s.v} {s.l}</span>
          ))}
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={sync} style={{fontSize:10,fontFamily:"monospace",background:"transparent",border:`1px solid ${C.border}`,borderRadius:20,padding:"4px 10px",color:C.muted,display:"flex",alignItems:"center",gap:5}}>
            <span style={{display:"inline-block",animation:syncing?"spin 1s linear infinite":"none"}}>↺</span>
            {syncing?"syncing":lastSync?timeAgo(lastSync):"sync"}
          </button>
          <button onClick={()=>setAdd(!showAdd)} style={{fontSize:12,fontFamily:"monospace",fontWeight:700,background:showAdd?"transparent":`linear-gradient(135deg,${C.pink},${C.purple})`,border:`1px solid ${showAdd?C.border:"transparent"}`,color:showAdd?C.muted:"#fff",padding:"7px 16px",borderRadius:20,transition:"all 0.2s"}}>
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"24px 20px 80px"}}>
        ...
        <div style={{display:"flex",gap:0,marginBottom:24,background:C.s1,borderRadius:12,padding:4,border:`1px solid ${C.border}`}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setView(t.id)} style={{flex:1,padding:"9px 0",border:"none",fontFamily:"monospace",fontSize:12,fontWeight:700,cursor:"pointer",borderRadius:10,transition:"all 0.2s",background:view===t.id?C.s2:"transparent",color:view===t.id?C.white:C.muted,boxShadow:view===t.id?`0 0 0 1px ${C.border}`:"none"}}>{t.label}</button>
          ))}
        </div>
        ...
        {reels.length===0&&!showAdd&&(
          <div style={{textAlign:"center",padding:"80px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
            <div style={{display:"flex",gap:12,opacity:0.5}}>
              <Star color={C.yellow} size={24}/><Asterisk color={C.mint} size={24}/><Star color={C.pink} size={20}/>
            </div>
            ...
            <p style={{fontSize:13,fontFamily:"monospace",color:C.muted,lineHeight:1.7,maxWidth:380}}>
              share any reel to the twilio whatsapp number.<br/>
              bot asks one thing. reply 1, 2, or 3. shows up here in 30s.
            </p>
            <Squiggle color={C.purple} w={100}/>
            <button onClick={()=>setAdd(true)} style={{padding:"10px 24px",borderRadius:20,border:"none",background:`linear-gradient(135deg,${C.pink},${C.purple})`,color:"#fff",fontWeight:700,fontSize:14}}>+ save first reel</button>
          </div>
        )}
